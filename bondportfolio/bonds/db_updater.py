...
response = requests.get('https://iss.moex.com/iss/engines/stock/markets/bonds/boards/TQCB/securities.json?iss.meta=off')
...
for bond in securities_data:
    print(bond)
    obj, created = Bonds.objects.update_or_create(
        name=bond[2][1],
        yield_date=bond[13][1],
        isin=bond[0][1]
    )
...
#         price=bond[2][1],
#         duration = bond[7][1],
#         effective_yield = bond[6][1],
#         g_spread = bond[9][1],
#         z_spread = bond[8][1],
#     )

#print(marketdata_yields_data)
